...
def validate_data(age, investmentAmount, riskLevel, intent_request):
    ...
    if age is not None:
        age_date = datetime.strptime(age, "%Y-%m-%d")
        date = relativedelta(datetime.now(), age_date).years
        if date > 0 and date < 65:
            return build_validation_result(
                False,
                "age",
                "You should be less than 64 years old to contract this service, "
    ...
    if investmentAmount is not None:
        ...
        if investmentAmount <= 5000:
            return build_validation_result(
                False,
                "investmentAmount",
                "The minimum investment amount is 50000, "
